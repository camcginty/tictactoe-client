...
var gameplayEvents = __webpack_require__(5);
...
var onCreateGame = function onCreateGame(event) {
  ...
  authApi.createGame().then(authUi.createGameSuccess).catch(authUi.createGameError);
};
...
var onGetGames = function onGetGames() {
  return authApi.getGames().then(authUi.getGamesSuccess).catch(authUi.getGamesError);
};
...
var updateGame = function updateGame(index, value, gameOver) {
  ...
  }).then(ui.updateGameSuccess).catch(ui.updateGameFail);
};
...
var signInError = function signInError(_signInError) {
  clearText();
...
var createGameSuccess = function createGameSuccess(_createGameSuccess) {
  clearText();
  $('#info').append('Make a move. You are both Xs and Os.');
  setTimeout(clearText, 2500);
  store.game = _createGameSuccess.game;
  document.getElementById('change-password-form').reset();
};

var createGameError = function createGameError(newGameError) {
  clearText();
...
var getGamesError = function getGamesError(games) {
  clearText();
...
var playHere = function playHere() {
  id = this.id;
...
var nextTurn = function nextTurn() {
  if (players[0].isTurn === true) {
    ...
    var _value = 'o';
    var _gameOver = false;
    api.updateGame(_index, _value, _gameOver);
  }
  if (players[0].isTurn === true) {
...
var sayWinner = function sayWinner() {
  if (players[0].isTurn === true) {
...
var endGame = function endGame() {
  if (players[0].isTurn === true) {
    var index = gameboard[id];
    var value = 'x';
    var gameOver = true;
    api.updateGame(index, value, gameOver);
  } else if (players[1].isTurn === true) {
    var _index2 = gameboard[id];
    var _value2 = 'o';
    var _gameOver2 = true;
    api.updateGame(_index2, _value2, _gameOver2);
  }
  players[0].isTurn = false;
  players[1].isTurn = false;
};
